...
Antes de avanzar vamos a asegurarnos que tengas una base sólida en ejecutar código R, y que conozcas algunas de las características más útiles de RStudio. 
...
Cuando leas esa línea de código mentalmente di "si pido objeto, obtengo valor "  

Escribirás montones de comandos de asignación y `<-` es incómodo de escribir. Que no te gane la pereza de usar `=`: sí, funcionará, pero provocará confusión después. En cambio, usa el atajo de teclado de RStudio Alt + - (signo menos). RStudio automágicamente rodeará `<-` con espacios, lo que es un buena costumbre para dar formato al código. El código puede ser horrible para leer incluso en un buen día, por lo que ayuda a tu vista usando espacios. 
...
Existe un acuerdo implícito entre tú y R: R hará todos los tediosos cálculos por ti, pero a cambio tu debes dar tus instrucciones con total precisión. Cada caracter es importante. Si está en mayúscula o minúscula es importante.
